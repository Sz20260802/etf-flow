...
背景：东财快照接口 fund_etf_spot_em 不含债券 ETF（实测 0 只），
     ...
     本脚本从中提取场内债券 ETF 补入 etf_info，并顺带把货币 ETF 单独归类。
...
MONEY_KEYWORDS = ["货币", "日利", "添益", "财富宝", "日日鑫", "天天金", "增益货币", "交易货币", "添利"]


def run() -> dict:
    """补全债券 ETF 档案，货币 ETF 重新归类。"""
    ...
    # 货币 ETF 单独归类（此前落在"其他"）
    money = query("SELECT code, name FROM etf_info")
    money = money[money["name"].str.contains("|".join(MONEY_KEYWORDS), na=False)]
    conn.executemany("UPDATE etf_info SET sector='货币' WHERE code=?",
                     [(c,) for c in money["code"]])
    conn.commit()
    conn.close()

    summary = {"bond_etf_found": len(bond_etfs), "new_added": len(new_rows),
               "money_reclassified": len(money)}
